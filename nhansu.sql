drop database if exists Nhansu;
...
select e.employee_id, first_name + " " + last_name As fullName, hire_date, department_name, job_title, min_salary, max_salary 
...
/*view 1. hiển thị danh sách các nhân viên có lương > 8000.00 gồm employee_id, first_name, email, phone_number, salary*/
drop view emp
CREATE VIEW emp AS
    select employee_id, first_name, email, phone_number, salary from employees
   where salary >8000.00 and salary<12000;

select *from emp


/*VIEW2: In ra danh sách các employees (employee_id, first_name, last_name, phone_number)
 có phone_number bắt đầu là “515” và kết thúc là “69”*/
drop view list_emp
CREATE VIEW list_emp AS
    SELECT employee_id, first_name, last_name, phone_number
   FROM  employees
   WHERE phone_number LIKE '515%69'

select *from list_emp


/**Proc 1: Tạo  Procedure để đếm tổng số nhân viên trong bảng employees**/
drop procedure count_employees
delimiter $$
CREATE PROCEDURE count_employees (OUT param1 INT)
  BEGIN
    SELECT COUNT(*) INTO param1, first_name, email   FROM employees;
  END;
 $$
 CALL count_employees(@full_employee);
SELECT @full_employee;

/* proc2: tạo proc_salaIncrease để tăng lương cho nhân viên: nếu nhân viên làm trên 20 năm thì thưởng 15000 còn lại thương 6000*/
delimiter $$
CREATE PROCEDURE proc_salaIncrease()

begin
select first_name ,  last_name , (YEAR(CURDATE()) - YEAR(hire_date)) AS years ,
case 
when (YEAR(CURDATE()) - YEAR(hire_date)) >= 20 then salary + 15000
else salary + 6000
end as salaIncrease FROM employees;
end;
$$
call proc_salaIncrease;
